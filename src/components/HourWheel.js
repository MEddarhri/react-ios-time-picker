...
function HourWheel({ height, value, setValue }) {
   const [hours, setHours] = useState(initialNumbersValue(height, 24, parseInt(value.slice(0, 2))));
   ...
   const [currentTranslatedValue, setCurrentTranslatedValue] = useState(
      parseInt(
         initialNumbersValue(height, 24, parseInt(value.slice(0, 2))).filter(
   ...
   useEffect(() => {
      if (showFinalTranslate) {
         ...
         if (dragEndTime - dragStartTime <= 100 && cursorPosition !== 0) {
            ...
            if (finalValue < height * -69) {
               finalValue = height * -69;
            }
   ...
   const handleTransitionEnd = () => {
      returnSelectedValue(height).map((item) => {
         if (parseInt(item.translatedValue) === currentTranslatedValue) {
            ...
            setHours(() => {
               const newValue = initialNumbersValue(height, 24).map((hour) => {
                  if (
